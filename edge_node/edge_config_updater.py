...
# WAF default variables (disabled by default, overridden per-domain if WAF enabled)
map $uri $waf_status {
    default "";
}
map $uri $waf_rule_id {
...
{% if domain.rate_limits %}
{% set safe_name = domain.name|replace('.', '_') %}
{% for rl in domain.rate_limits %}
limit_req_zone $binary_remote_addr zone={{ safe_name }}_rl_{{ rl.id }}:10m rate={{ (rl.limit_value / rl.interval_seconds)|int|default(10) }}r/s;
{% endfor %}
{% endif %}
{% endfor %}
...
server {
    ...
    {% if waf_rule.conditions_parsed %}
    {% if waf_rule.conditions_parsed.path %}
    location ~ {{ waf_rule.conditions_parsed.path }} {
        {% if waf_rule.action == 'block' %}
        set $waf_status "blocked";
        set $waf_rule_id "{{ waf_rule.id }}";
        return 403 "Blocked by WAF rule {{ waf_rule.id }}";
        {% elif waf_rule.action == 'challenge' %}
        set $waf_status "challenged";
        set $waf_rule_id "{{ waf_rule.id }}";
        return 503;
        {% endif %}
    }
    ...
    {% for rl in domain.rate_limits %}
    {% if rl.path_pattern %}
    location ~ {{ rl.path_pattern }} {
        limit_req zone={{ safe_name }}_rl_{{ rl.id }} burst={{ ((rl.limit_value / rl.interval_seconds) * 2)|int|default(20) }} nodelay;
        limit_req_status {{ rl.response_status|default(429) }};
